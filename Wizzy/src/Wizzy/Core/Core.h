...
#define WZ_VERSION_SUM(major, minor, patch)		(major * 1000 + minor * 100 + patch)
	...
	#define typecstr(T) (abi::__cxa_demangle(typeid(T).name(), 0, 0, 0))
#else
	#define typestr(T)  (string(typeid(T).name()))
	#define typecstr(T) (typeid(T).name())
#endif
...
#define WZ_NULL_ENTITY_HANDLE 0
...
#else
    #define force_inline inline
    #warning force_inline could not be defined for compiler. Performance impact is possible.    
#endif